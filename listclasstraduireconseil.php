...
Veuillez sélectionner une classe pour avoir accès à la liste des élèves afin de traduire un ou plusieurs élèves au conseil de discipline.
</div>';
  ...
  <tr><th>N°</th><th> Nom de la classe</th><th>Section</th></tr>
    ...
    if($sel = $connec->query($se)){
        while($sele = $sel->fetch()){
            ...
            echo '<tr id="'.$sele['id_classe'].'" onclick="listelevetraduit(this.id)" style="cursor:pointer"><td>'.$n.'</td><td>'.$sele['nom_classe'].'</td><td>'.$sele['nom_section'].'</td></tr>';
            $n++;
        }
    }
