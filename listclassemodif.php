...
<thead class="table-primary">
  <tr><th></th><th>N°</th><th> Nom de la classe</th><th>Montant scolarité</th><th>Montant inscription</th><th>Section</th></tr>
  </thead>';
if($search == ''){
    ...
    if($sel = $connec->query($se)){
        while($sele = $sel -> fetch()){
            ...
            echo '<tr ><td style="cursor:pointer;color:gray;width:100px" id="'.$sele['id_classe'].'" onclick="formclassemodif(this.id)"><i class="bi bi-pencil-fill text-warning"></i> Modifier</td><td>'.$n.'</td><td>'.$sele['nom_classe'].'</td><td>'.$sele['montscolarite'].'</td><td>'.$sele['montinscription'].'</td><td>'.$sele['nom_section'].'</td></tr>';
            $n++;
        }
    }
}else if($search != ''){
