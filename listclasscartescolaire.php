...
Veuillez sélectionner une classe pour générer les cartes scolaires des élèves de cette classe.
</div>';
  ...
  <tr><th>N°</th><th> Nom de la classe</th><th>Section</th></tr>
    ...
    if($sel = $connec->query($se)){
        while($sele = $sel->fetch()){
            ...
            echo '<tr id="'.$sele['id_classe'].'" onclick="affichcartescolaire(this.id)" class="linejustabsent" style="cursor:pointer"><td>'.$n.'</td><td>'.$sele['nom_classe'].'</td><td>'.$sele['nom_section'].'</td></tr>';
            $n++;
        }
    }
