...
<body>
    <div style="color:#0d6efd;border-bottom:1px solid #0d6efd">
        <i class="bi bi-arrow-left-short" style="font-size:40px; margin-left:20px; vertical-align:middle;cursor:pointer" onclick="window.history.go(-1)"></i>&nbsp;&nbsp;  <strong style="font-size:20px;vertical-align:middle">Mon Emploi du temps</strong> 
    </div>
    <?php
require('connect.php');
$ident = $_GET['id'];
...
//traitement pour afficher les heures de pausse sur l'emploi de temps
...
if($kie = $connec -> query($ki)){
    while($kiem = $kie -> fetch()){
        ...
        echo '<br/><br><table style="text-align:center;border-collapse:collapse;font-size:14px;margin:0 auto; width:95%" border="1">';
