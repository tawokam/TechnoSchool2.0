...
<tr style="text-align:left">
    ...
    <td colspan="2">
        <?php
$vres = "SELECT inscription.matricule,inscription.montinscription,caisseinscription.id_caisseinscript,caisseinscription.montverse,sum(caisseinscription.montverse) as montotverse,caisseinscription.matricule,caisseinscription.nom_session FROM inscription inner join caisseinscription on inscription.matricule = caisseinscription.matricule where id_caisseinscript <= '$ident' AND caisseinscription.matricule='$matricule' AND caisseinscription.nom_session='$session'";
if($vrest = $connec -> query($vres)){
    while($vreste = $vrest -> fetch()){
        $sompaye = $vreste['montotverse'];
        $rest = $vreste['montinscription'] - $sompaye;
