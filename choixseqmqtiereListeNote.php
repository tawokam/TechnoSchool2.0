...
if($coe = $connec -> query($co)){
    while($coef = $coe -> fetch()){
        ...
        <div class="col-md-12">
        <label for="coefinsertmodif" class="form-label">Coéficient : '.$coef['coef'].'</label>
        </div></form><br>
        ';
    }
}
...
if($sel = $connec -> query($se)){
    while($sele = $sel -> fetch()){
        ...
        <tr ><td><?php echo $nbr;?></td><td><?php echo $sele['matricule'];?></td><td><?php echo $sele['nom_eleve'];?></td><td><?php echo $sele['note'];?></td></tr>
<?php
$nbr++;
$lignepresent = 2;
    }
