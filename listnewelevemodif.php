...
<thead class="table-primary">
  <tr><th></th><th>N°</th><th> Nom de l\'élève</th><th>Matricule</th><th>Date de naissance</th><th>Date d\'entré</th><th>Sexe</th><th>Nationalité</th><th>Adresse</th><th>Nom du tutteur</th><th>Téléphone tutteur</th></tr>
  </thead>';
if($sexe == '' && $nom == ''){
    ...
    if($sel = $connec -> query($se)){
        while($sele = $sel -> fetch()){
            ...
            echo '<tr ><td style="cursor:pointer;color:gray;width:100px" id="'.$sele['id_eleve'].'" onclick="formnewelevemodif(this.id)"><i class="bi bi-pencil-fill text-warning"></i> Modifier</td><td>'.$n.'</td><td>'.$sele['nom_eleve'].'</td><td>'.$sele['matricule'].'</td><td>'.$sele['datenaiss_eleve'].'</td><td>'.$sele['dateentre_eleve'].'</td><td>'.$sele['sexe_eleve'].'</td><td>'.$sele['nationalite_eleve'].'</td><td>'.$sele['adresse_eleve'].'</td><td>'.$sele['nomtitteur'].'</td><td>'.$sele['telephonetitteur'].'</td></tr>';
            $n++;
        }
    }
}else if($sexe != '' && $nom == ''){
